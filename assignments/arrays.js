...
function  findCarId(idNum){
  for (i=0; i<inventory.length; i++){
    if (inventory[i].id === idNum){
      console.log(`Car ${idNum} is a ${inventory[i].car_year} ${inventory[i].car_make} ${inventory[i].car_model}`)
    }
  }
}
...
function findCarId2(idNum){
  const idCar = inventory.filter( (car) => car.id === idNum );
  console.log (`Car ${idNum} is a ${idCar[0].car_year} ${idCar[0].car_make} ${idCar[0].car_model}`)
}
...
console.log(`Last car: ${lastCar.car_year} ${lastCar.car_make} ${lastCar.car_model}`);
...
//STRETCH
let carModels2 = [];
inventory.map(car => carModels2.push(car.car_model));
...
//STRETCH
let carYears2 = [];
inventory.map(car => carYears2.push(car.car_year));
...
//STRETCH
let BMWCars = inventory.filter(function (car){
  if (car.car_make === 'BMW'){
    return true;
  } else {
    return false;
...
let audiCars = inventory.filter(function (car){
...
let BMWAndAudi2 = BMWCars.concat(audiCars);
